...
#### duplicates - 查找重复项
```bash
mediaorganizer index duplicates [OPTIONS]
...
#### collections - 电影系列合集管理
```bash
mediaorganizer index collections [OPTIONS]
...
#### tv - 电视剧统计管理
```bash
mediaorganizer index tv [OPTIONS]
...
```bash
# 场景 1: 添加新文件或修改现有文件
mediaorganizer index scan --force /path/to/media --volume-label Disk_Movies_01 --media-type movies
